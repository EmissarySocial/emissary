...
Two further notes on the write paths. `Save` is reached *only* from the settings form, so there are no background writes to keep off the network — which is why an active connection is re-proved on every save, and why that re-run is what restores anything the User deleted by hand at the far end. And a queue task that discovers a rejected credential must write `Status` through `collection.Save` directly, **not** through `UserConnection.Save`, which would call `connect()` and reach for the same credential that was refused moments before.
...
The second problem is in templates: a Go template that renders an optional header emits the literal string `<no value>` for a missing key rather than nothing, which produces a malformed header instead of an absent one. Guard the whole header line with `{{if}}`; guarding the value alone still emits an empty header.
...
## An invalid signature refuses the request, and three rules keep that refusal accurate
...
**`Following.SetStatusPollError` owns the whole meaning of a failed Actor load.** In [following_pollError.go](following_pollError.go) it chooses `GONE` on a `410` and `FAILURE` otherwise (escalating to `PAUSED` through `SetStatusPollFailure`), and writes the sentence the owner reads. Callers pass the raw error and never pick a status or write the text themselves; if two packages each decided part of the lifecycle, a change to one would leave the other describing states it no longer produces.

**A `429` must never reach it.** A rate limit is the host's throttle, so callers requeue it first. If one arrives anyway, the method records it as the record's own failure.

**Every Following setter that takes a caller's message bounds it to the 1024-byte `statusMessage` schema.** `SetStatusGone`, `SetStatusPollFailure`, and `SetStatusFailure` all call `truncateStatusMessage`, as `StreamSource.SetStatusFailure` does. A message that quotes a remote root message has no bound of its own, and the schema rejects a longer one.
