...
func (adapter IPAPICO) ManualConfig() form.Form {
			...
			Children: []form.Element{
				{
					...
					Children: []form.Element{
						{
							Type:        "html",
							Description: "Returns geocoded location data for each IP address. No signup required to use free tier.  Or pay for increased rate limits at <a href='https://ipapi.co/'>ipapi.co</a>.",
						},
						{
							Type:    "hidden",
							Path:    "type",
...
func (adapter IPAPICO) Connect(connection *model.Connection, vault mapof.String, host string) error {
	return nil
}

// Refresh updates this connection if it has changed or is out of date
func (adapter IPAPICO) Refresh(connection *model.Connection, vault mapof.String) error {
	return nil
}

// Disconnect applies any extra changes to the database when this Adapter is disconnected
func (adapter IPAPICO) Disconnect(connection *model.Connection, vault mapof.String) error {
	return nil
}
