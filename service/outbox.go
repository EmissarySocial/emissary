...
func (service *Outbox) Query(criteria exp.Expression, options ...option.Option) ([]model.Activity, error) {
...
func (service *Outbox) LoadItemByID(ownerID primitive.ObjectID, outboxItemID primitive.ObjectID, result *model.Activity) error {

	criteria := exp.
		Equal("_id", outboxItemID).
...
func (service *Outbox) LoadByDocumentURL(ownerID primitive.ObjectID, documentURL string, result *model.Activity) error {
