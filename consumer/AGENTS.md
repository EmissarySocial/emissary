...
**A 429's `Retry-After` survives `derp.Wrap` and nothing else.** `derp.ErrorCode` falls back to `errors.As`, so an HTTP status climbs through any foreign wrapper. `derp.RetryAfter` does not — it is a single type assertion, and `derp.Error.GetRetryAfter` propagates only by delegating to its own `WrappedValue`. Put one `fmt.Errorf("%w")` anywhere in the chain and the duration silently becomes zero, at which point `derp.IsTooManyRequests` still answers TRUE (the code survived) and substitutes its one-hour default. A host asking for 30 seconds gets deferred an hour, and nothing logs the difference. The repo-wide rule against `fmt.Errorf` is what keeps the chain intact; this is what it costs when it is broken.
