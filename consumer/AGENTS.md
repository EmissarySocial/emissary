...
**A dead domain never answers 4xx.** It answers DNS failure, connection refused, TLS handshake failure, or timeout, and `derp` reports all of those as 500. So `derp.IsClientError` is the wrong gate for "did this attempt complete?" — an earlier cut classified only client errors and left exactly the dead-domain case touching the record at all, churning an error-log entry every sweep forever. `actorError` now records **every** completed attempt and reserves `derp.Report` for what nothing can account for, which excludes a 4xx and a 2xx that simply was not an Actor.
