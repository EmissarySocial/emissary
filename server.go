...
func makeStandardRoutes(factory *server.Factory, e *echo.Echo) {
	...
	e.GET("/.unsplash/photos/:photo", handler.WithAuthenticatedUser(factory, unsplash.GetPhoto))
	e.GET("/.unsplash/collections/:collection/random", handler.WithAuthenticatedUser(factory, unsplash.GetCollectionRandom))
	...
	e.GET("/@:userId/pub/keyPackages", handler.WithAuthorizedActorAndUser(factory, ap_user.GetKeyPackageCollection))
	e.GET("/@:userId/pub/keyPackages/:keyPackageId", handler.WithAuthorizedActorAndUser(factory, ap_user.GetKeyPackageRecord))
