...
func pipeline(ctx context.Context, collection data.Collection, result any, pipeline bson.A, opts ...*options.AggregateOptions) error {
	...
	if mongo == nil {
		return derp.Internal("queries.pipeline", "Database must be MongoDB")
	}
	...
	if err != nil {
		return derp.Wrap(err, location, "Querying database")
	}

	// Execute the query.  Results returned in "result" pointer
	if err := cursor.All(ctx, result); err != nil {
		return derp.Wrap(err, location, "Reading results")
	}
