...
func (step StepTableEditor) Post(builder Builder, _ io.Writer) PipelineBehavior {
	...
	if edit := builder.QueryParam("edit"); edit != "" {
		...
		if !ok {
			return Halt().WithError(derp.Internal(location, "Failed to convert edit index", step.Path, edit))
		}

		if editIndex < 0 {
			return Halt().WithError(derp.Internal(location, "Edit index out of range", step.Path, editIndex))
		}
		...
		if (deleteIndex < 0) || (deleteIndex >= convert.SliceLength(table)) {
			return Halt().WithError(derp.Internal(location, "Edit index out of range", step.Path, deleteIndex))
		}

		// Try to find the schema element for this table control
		if ok := builder.schema().Remove(builder.object(), step.Path+"."+delete); !ok {
			return Halt().WithError(derp.Internal(location, "Failed to remove row from table", step.Path))
		}
	}
