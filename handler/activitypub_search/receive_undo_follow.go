...
func undoFollow(context Context, activity streams.Document) error {
	...
	if err := followerService.LoadByActivityPubFollower(model.FollowerTypeSearch, context.searchQuery.SearchQueryID, actorURL, &follower); err != nil {

		if derp.IsNotFound(err) {
			return nil
		}

		return derp.Wrap(err, location, "Error locating follower", activity.Value(), context.searchQuery.SearchQueryID, actorURL)
	}
