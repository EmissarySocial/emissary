...
## A published activity with no `id` is worse than no activity at all

`CollectionPage` and `CollectionPage_Links` ([activitypub/collection.go](activitypub/collection.go)) drop any item whose `id` is empty, and the single-item outbox routes 404 rather than answer with one. An activity with no `id` cannot be dereferenced, deduplicated, or referenced back by any peer, so serving it does not degrade gracefully — it breaks every consumer that reads the page, including Emissary's own poller.

The failure this guards against is a *concatenation*, not a missing field. `OutboxMessage.ActivityPubURL()` builds `<actorURL>/pub/outbox/<id>`, so an empty `ActorURL` silently yields the relative path `/pub/outbox/<id>` — which looks like a URL, serializes without complaint, and is refused by `hannibal.streams.document.Load` at the far end. 78 such records once generated 7,504 errors in a week (BUG-146). `ActivityPubURL()` now returns empty instead, and the page builders are what keep an empty value from reaching the wire.

Two details to preserve. Paging math reads the QUERY results (`len(values)`), never the rendered `OrderedItems` — paging from the rendered list would let one omitted item end the collection at whatever page happened to contain it. And the omission belongs in the shared builders, not in each of the four `activitypub_*` packages, for the reason [activitypub/doc.go](activitypub/doc.go) gives about every other collection rule.
...
- `uri.IsValidRedirectURL` is the STRICTER gate, for a destination named by somebody else. `GetOutboundIntent` ([intent_outbound.go](intent_outbound.go)) forwards a visitor to a URL template published by their home server's WebFinger, and the `account` query param decides WHICH server is asked — so a crafted link picks the server that picks the destination. The guard demands an absolute http(s) URL, which is what stops a template of `/signout` or `/\phish.example` bouncing the visitor back inside this origin. An off-site host is deliberately NOT rejected: sending the visitor to their own home server is the entire feature, so this endpoint is a caller-steered redirect by design and a confirmation interstitial, not a validator, is the only thing that would change that. **Keep the call in the positive form** (`if !uri.IsValidRedirectURL(…)`): CodeQL's `go/unvalidated-url-redirection` recognises a sanitizer only by callee name, and the `NotValidRedirectURL` twin does not match its pattern, so the `Not…` spelling is correct code that silently leaves the alert open. The uri package's own AGENTS.md carries the pattern.
- `PostSearchLookup` ([search.go](search.go)) redirects only to this server's own SearchQuery Actor. It used to prefix that with a caller-supplied `forward` query param, which was a working open redirect for anyone who could get a same-origin page to POST to it; nothing in the tree ever sent the parameter, and it was removed. Do not reintroduce a caller-supplied prefix — the referer check above it guards against forged requests, not against bad destinations.
