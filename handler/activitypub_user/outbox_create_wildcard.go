...
// outbox_CreateAny creates a new note (now, just a private message) in the user's outbox
func init() {

	// Removing this for now, in favor of outbox_Wildcard, which does not create an Object record in the database.
	// outboxRouter.Add(vocab.ActivityTypeCreate, vocab.Any, outbox_CreateWildcard)
}

func outbox_CreateWildcard(context Context, document streams.Document) error {

	const location = "handler.activitypub_user.outbox_CreateAny"
