...
func GetGeosearchAsOptions(ctx *steranko.Context, factory *service.Factory, session data.Session) error {

	const location = "handler.GetGeosearch"

	// Query the Geosearch service (allow any parameter to work..)
	query := ""
	...
	// Use the GeoSearch service to search for places matching this query
	geosearchService := factory.Geosearch()
	results, err := geosearchService.Search(session, query, ctx.Request().Referer())
