...
type Email struct {
	EmailID   string                 `json:"emailId"`   // Unique identifier for this email.
	EmailRole string                 `json:"emailRole"` // Role of the email - for system emails that may have multiple options
	Model     string                 `json:"model"`     // Object type that this email is associated with
	Headers   *texttemplate.Template `json:"-"`         // Additional email header values (plain-text context)
	To        *texttemplate.Template `json:"-"`         // Template for the email address to send this email to (plain-text context)
	Subject   *texttemplate.Template `json:"-"`         // Template for the email subject (plain-text context)
	Body      *htmltemplate.Template `json:"-"`         // Template for the HTML email body. This uses html/template so that
	...
	Resources fs.FS `json:"-"` // File system containing additional files (like images) required by this email
}
