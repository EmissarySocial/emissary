...
type Rule struct {
	...
	Type            string             `json:"type" bson:"type"`                                 // Type of Rule (e.g. "ACTOR", "DOMAIN", "TAG")
	...
	MatchKey        string             `json:"matchKey" bson:"matchKey"`                         // Derived "<TYPE>:<normalized trigger>" -- the indexed key that a matching document also produces. Computed in Save from Type+Trigger; NEVER set from a form (see RuleSchema).
	RemoteID        string             `json:"remoteId" bson:"remoteId,omitempty"`               // Canonical id (URL) of the remote moderation entry this Rule was imported from (P7-3). "" = created locally. Server-set only -- NEVER in RuleSchema.
	ReasonCode      string             `json:"reasonCode" bson:"reasonCode"`                     // Optional code to identify the reason for this rule (e.g. "SPAM", "NSFW", "SENSITIVE")
	...
	PublishDate     int64              `json:"publishDate" bson:"publishDate"`                   // Unix epoch SECONDS when this rule was published to followers (0 = unpublished)
	PublishedAction string             `json:"publishedAction" bson:"publishedAction,omitempty"` // The Action as LAST PUBLISHED (D15/P7-2): retractions embed this, never the live Action, which may have changed since. "" = never published (or retracted). Server-set only -- NEVER in RuleSchema.
	ExpireDate      int64              `json:"expireDate" bson:"expireDate"`                     // Unix epoch SECONDS when this rule expires and becomes inert (0 = never)
