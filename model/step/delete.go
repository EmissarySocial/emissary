...
func NewDelete(stepInfo maps.Map) (Delete, error) {
	...
	titleTemplate, err := template.New("").Parse(first.String(getValue(stepInfo.GetString("title")), "Delete '{{.Label}}'?"))

	if err != nil {
		return Delete{}, derp.Wrap(err, "model.step.NewDelete", "Invalid 'title' template", stepInfo)
	}

	messageTemplate, err := template.New("").Parse(first.String(getValue(stepInfo.GetString("message")), "Are you sure you want to delete {{.Label}}? There is NO UNDO."))
	...
	return Delete{
		...
		Submit:  first.String(getValue(stepInfo.GetString("submit")), "Delete"),
	}, nil
}
