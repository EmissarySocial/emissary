...
**Name a variable for its type, never for where it came from.** A `Domain` is `readOnlyDomain` and a `WritableDomain` is `writableDomain`, including parameters, receivers, and the builders' `_readOnlyDomain`/`_writableDomain` fields. Provenance is a property of the value, not the type: the same record is loaded, published to the cache, and handed to a reader, and two earlier naming schemes (`CachedDomain`/`StoredDomain` among them) were false somewhere for that reason. The exceptions are a function holding two values of one type, which keeps role names (`before`/`after`), and the published snapshot in `service.Domain.Cached()` and `publish` (`result`, `blank`, `clone`), which must never be written and so is not called writable. Rationale: [DOMAIN-READ-WRITE-SPLIT](../../emissary-specs/projects/DOMAIN-READ-WRITE-SPLIT.md) D15.
