...
The absence of a `json` tag is not a decision — it is the default, and the default is "export it". `GET /@:userId/export/:collection/:recordId` marshals model objects straight to JSON, and because `model.User` and `model.Following` carried only `bson` tags, that route emitted `User.Password` (the hash), `User.PasswordReset` (a live account-takeover code), and `Following.Secret` (the remote feed credential). The route is reachable with a third-party OAuth grant, so any app authorized for portability could harvest all three. They now carry `json:"-"`, pinned by `service/export_credentials_test.go`. **Any new secret-bearing field on a model needs an explicit `json:"-"`**, and any new route that marshals a model wholesale needs to be checked against this.
