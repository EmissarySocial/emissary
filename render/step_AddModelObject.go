...
func (step StepAddModelObject) Get(renderer Renderer, buffer io.Writer) ExitCondition {
	...
	status := Pipeline(step.Defaults).Get(factory, renderer, buffer)

	if status.Halt {
		status.Error = derp.Wrap(status.Error, "render.StepAddModelObject.Get", "Error executing default steps")
		return ExitWithStatus(status)
	}

	// Try to render the Form HTML
	result, err := form.Editor(schema, step.Form, object, renderer.lookupProvider())

	if err != nil {
		return ExitError(derp.Wrap(err, "render.StepAddModelObject.Get", "Error generating form"))
	}
...
func (step StepAddModelObject) Post(renderer Renderer, buffer io.Writer) ExitCondition {
	...
	status := Pipeline(step.Defaults).Post(factory, renderer, buffer)

	if status.Halt {
		status.Error = derp.Wrap(status.Error, "render.StepAddModelObject.Post", "Error executing default steps")
		return ExitWithStatus(status)
	}

	// Parse form information
	if err := request.ParseForm(); err != nil {
		return ExitError(derp.Wrap(err, "render.AddModelObject.Post", "Error parsing form data"))
	}
	...
	for key, value := range request.Form {
		if err := schema.Set(object, key, value); err != nil {
			return ExitError(derp.Wrap(err, "render.AddModelObject.Post", "Error setting path value", key, value))
		}
	}

	// Save the object to the database
	if err := renderer.service().ObjectSave(object, "Created"); err != nil {
		return ExitError(derp.Wrap(err, "render.StepAddModelObject.Post", "Error saving model object to database"))
	}
